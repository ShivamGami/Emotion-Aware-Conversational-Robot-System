...
SMOOTHER_WINDOW = 5         # number of past frames kept for mode smoothing
...
class EmotionSmoother:
    ...
    stable_emotion = smoother.update("happy")
    """
    ...
    def update(self, raw_emotion: str) -> str:
        ...
        If the window is not yet full, returns the mode of whatever is stored so
        far (falls back to raw_emotion when only one reading exists).
        """
        self._history.append(raw_emotion)
        try:
            return mode(self._history)          # most frequent in window
        except Exception:
            return raw_emotion                  # tie-break: return raw
...
class FaceEmotionDetector:
    ...
    Backend: "deepface" (default)
        Uses DeepFace + OpenCV Haar Cascades (Task 1.1 behaviour).
    Backend: "custom_cnn"
    ...
    ----------
    backend:
        "deepface" or "custom_cnn".
    weights_path:
        Path to the .pth weights file. Only used when backend="custom_cnn".
        Falls back gracefully to DeepFace if file is not found.
    deepface_detector:
        DeepFace internal detector backend (opencv / retinaface / …).
    enforce_detection:
        If False, returns neutral on no-face instead of raising.
    smoother_window:
        Number of past frames used by EmotionSmoother.
    """
    ...
    def __init__(
        ...
        logger.info(
            "FaceEmotionDetector initialised ✓  [backend=%s | device=%s]",
            self._backend,
            self._device,
        )
    ...
    def detect_from_frame(self, frame: np.ndarray) -> EmotionResult:
        ...
        # Presence check via Haar Cascade
    ...
    def compare_both_models(self, frame: np.ndarray) -> dict:
        ...
        Used by POST /api/compare_models.

        Returns
        -------
        dict with keys: "deepface", "custom_cnn", "agreement"
        """
    ...
    def _run_haar(self, frame: np.ndarray) -> tuple[bool, Optional[FaceRegion]]:
        """Quick face-presence check using OpenCV Haar Cascades."""
